class User < ActiveRecord::Base
  ...
  def self.most_orders_placed
    User.find_by_sql("SELECT users.first_name, users.last_name, count(order_id) AS sum 
      FROM products 
      JOIN order_contents ON products.id=order_contents.product_id
      JOIN orders ON orders.id = order_contents.order_id 
      JOIN users ON orders.user_id = users.id 
      GROUP BY order_id ORDER BY sum DESC LIMIT 1")
  end
end
